...
import { usePageHeader, usePeriod, useUser, useToast } from '../../hooks';
...
import type { MyCommittee, CommitteeMemberInfo, CommitteeMemberRole, CommitteeMemberGradeData, RegistrationGradeData } from '../../types';
...
export default function CommitteeDetail() {
  ...
  const periodId = statePeriodId || (period?.id ? String(period.id) : undefined);
  ...
  usePageHeader({
    ...
    description: committee?.name || 'Đang tải...',
  });
  ...
  useEffect(() => {
    if (selectedRegId) {
      loadRegistrationData(selectedRegId);
    } else {
      setRegDetail(null);
      setFinalReport(null);
  ...
  // Khi bật "Xem báo cáo", gọi API lấy presigned URL của file PDF
  useEffect(() => {
    if (reportViewOpen && finalReport?.latest?.id) {
      loadReportPdfUrl(finalReport.latest.id);
    } else {
  ...
  const myGrade = currentGradeData?.committee_members?.find((m) => m.lecturer_id === user?.id) ?? null;
  ...
  const loadGrade = (regId: number): Promise<RegistrationGradeData | null> => {
    return new Promise((resolve) => {
      fetchWithAuth(
        ...
        (type, msg) => {
          toast.error('Lỗi', type === 'network' ? 'Không thể kết nối.' : msg || 'Không tải được điểm.');
          resolve(null);
        },
        {}
      );
  ...
  const loadRegistrationData = async (regId: number) => {
    ...
    const [regRes, reportRes] = await Promise.allSettled([
      new Promise((resolve) => {
        fetchWithAuth(
          endpoints.registrationDetail(periodId, regId),
      ...
      new Promise((resolve) => {
        fetchWithAuth(
          `${endpoints.finalReport}?registration=${regId}`,
          (data: any) => resolve(data),
          () => resolve(null),
          {}
        );
  ...
  const loadDetail = async () => {
    ...
    setLoading(true);
    await fetchWithAuth(
      endpoints.committeeDetail(periodId, id),
      (data: any) => {
        setCommittee({
          ...data,
          members: data.members_detail || [],
          registrations: data.registrations_detail || [],
          role_in_committee: data.role_in_committee ?? data.role ?? 'member',
        });
      },
      () => { },
      {},
  ...
  const loadReportPdfUrl = async (reportId: number) => {
    ...
    await fetchWithAuth(
      ...
      (data: any) => {
        // Chỉnh lại field này nếu response backend khác {url: "..."}
        const url = data?.url || data?.download_url || (typeof data === 'string' ? data : null);
        if (url) {
          setReportPdfUrl(url);
        } else {
          setPdfError('Không lấy được đường dẫn file.');
        }
      },
  ...
  const handleSaveGrade = async (regId: number, score: number, comment: string) => {
    ...
    const onSuccess = async () => {
      toast.success(isEdit ? 'Cập nhật thành công' : 'Gửi thành công', isEdit ? 'Điểm đã được cập nhật.' : 'Điểm đã được gửi.');
    ...
    if (isEdit) {
      await updatePatchWithAuth(
        endpoints.gradeDetail(myGrade.id),
        { score, comment },
        onSuccess,
        onError,
        setSubmittingGrade
  ...
  if (!committee) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-gray-400">
        ...
        <p className="text-sm font-medium">Không tìm thấy hội đồng</p>
        <Button variant="outline" size="sm" className="mt-4" onClick={() => navigate('/my-committees')}>
          Quay lại danh sách
        </Button>
      </div>
    );
  ...
  const memberColumns: TableColumn<CommitteeMemberInfo>[] = [
    ...
    },
    {
      key: 'lecturer_name',
      label: 'Họ tên',
      render: (row) => <span className="font-medium text-gray-800">{row.lecturer_name}</span>,
    },
    {
      key: 'role',
      label: 'Vai trò',
      render: (row) => {
        const cfg = COMMITTEE_ROLE_CONFIG[row.role];
        return <Badge variant={cfg?.variant || 'neutral'}>{cfg?.label || row.role}</Badge>;
      },
    },
  ];

  const committeeGradeColumns: TableColumn<CommitteeMemberGradeData>[] = [
    {
      key: 'stt',
      label: 'STT',
      align: 'center',
    ...
    },
    {
      ...
      label: 'Thành viên',
      render: (row) => {
        const cfg = row.role ? COMMITTEE_ROLE_CONFIG[row.role as CommitteeMemberRole] : null;
        return (
          <div>
            <div className="font-medium text-gray-800">{row.member_name || 'Không rõ'}</div>
            {cfg && <Badge variant={cfg.variant} className="mt-1 text-[10px]">{cfg.label}</Badge>}
          </div>
        );
      },
    },
    {
      ...
      render: (row) =>
        row.score ? (
          <span className="font-semibold text-gray-800">{row.score}</span>
        ) : (
    ...
    },
    {
      ...
      label: 'Nhận xét',
      render: (row) => (
        <span className="text-gray-600 text-xs line-clamp-2">{row.comment || '—'}</span>
      ),
    },
    {
      key: 'graded_at',
      label: 'Thời gian',
      render: (row) => (
        <span className="text-gray-500 text-xs whitespace-nowrap">{row.graded_at ? formatDate(row.graded_at) : '—'}</span>
      ),
    },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-textMain">{committee.name}</h2>
            <div className="flex flex-wrap items-center gap-2 mt-1">
              <Badge variant={statusCfg?.variant || 'neutral'} dot>{statusCfg?.label}</Badge>
              <Badge variant={roleCfg?.variant || 'neutral'}>{roleCfg?.label}</Badge>
            </div>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500">
          <div className="flex items-center gap-2">
            ...
            <span>{formatDate(committee.defense_date)}</span>
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span>{committee.period_name}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-10 gap-6">
        ...
        <div className="lg:col-span-3 space-y-6">
          {/* Danh sách đề tài */}
          <Card variant="elevated" icon="fa-solid fa-file-lines" title={`Đề tài (${registrations.length})`}>
            <div className="space-y-2 max-h-[370px] overflow-y-auto pr-1">
              {registrations.length === 0 ? (
                ...
                registrations.map((reg) => {
                  ...
                  const regStatus = STATUS_CONFIG[reg.status || ''];
                  return (
                    <div
                      key={reg.id}
                      onClick={() => setSelectedRegId(reg.id)}
                      className={`p-3 rounded-xl cursor-pointer border transition-all duration-150 ${isSelected
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        {regStatus && (
                          <Badge variant={regStatus.variant} className="shrink-0 text-[10px]">
                            {regStatus.label}
                          </Badge>
                        )}
                      </div>
          ...
          {/* Thành viên hội đồng */}
          <Card variant="elevated" icon="fa-solid fa-user-group" title="Thành viên hội đồng">
            <GenericTable
              rows={members}
        ...
        <div className="lg:col-span-7">
          {!selectedReg ? (
            ...
            <div className="space-y-6">
              {/* Thông tin đề tài + Báo cáo cuối kỳ */}
                    ...
                    <div className="space-y-1">
                      <div className="pb-3 border-b border-gray-100">
                        <h3 className="text-base font-bold text-textMain">
                          {regDetail?.project_title || selectedReg.project_title}
                        </h3>
                        <div className="flex items-center gap-3 mt-2">
                          <span className="text-sm text-gray-600">
                            <i className="fa-solid fa-user-graduate text-gray-400 mr-1"></i>
                            {regDetail?.student_name || selectedReg.student_name}
                          </span>
                          <span className="text-gray-300">·</span>
                          <span className="font-mono text-xs">
                            {regDetail?.student_id || selectedReg.student_id}
                          </span>
                        </div>
                      </div>
                      <InfoTile
                        ...
                        label="Mô tả"
                        value={regDetail?.project_description || selectedReg.description || 'Chưa có mô tả'}
                      />
                      <InfoTile
                        icon="fa-solid fa-user-tie"
                        label="Giảng viên hướng dẫn"
                        value={regDetail?.lecturer_name || '—'}
                      />
                      <InfoTile
                        icon="fa-solid fa-graduation-cap"
                        label="Trạng thái"
                        value={regDetail?.status_display || selectedReg.status}
                      />
                      {regDetail?.wants_thesis_upgrade && (
                        <InfoTile icon="fa-solid fa-arrow-up" label="Nâng cấp khóa luận" value="Có" />
                      )}
                      <div className="pt-2">
                        <Button
                          variant="outline"
                  ...
                  <SectionCard title="Báo cáo cuối kỳ" icon="fa-solid fa-file-pdf">
                    <div className="space-y-1">
                      ...
                      <InfoTile
                        ...
                        label="Kích thước"
                        value={`${(finalReport.latest.file_size / 1024).toFixed(1)} KB`}
                      />
                      <InfoTile
                        icon="fa-solid fa-clock"
                        label="Ngày nộp"
                        value={formatDate(finalReport.latest.created_date)}
                      />
                      {(() => {
                        ...
                        return (
                          <InfoTile
                            ...
                            label="Trạng thái"
                            value={
                              <Badge variant={st?.variant || 'neutral'} dot>
                                {st?.label || finalReport.latest.status}
                              </Badge>
                            }
                          />
                        );
                      })()}
                      {finalReport.latest.feedback && (
                        <InfoTile icon="fa-solid fa-comment" label="Phản hồi" value={finalReport.latest.feedback} />
                      )}
                      <div className="pt-2">
                        <Button
                          variant="outline"
                          size="sm"
                          icon={reportViewOpen ? 'fa-solid fa-eye-slash' : 'fa-solid fa-expand'}
                          onClick={() => setReportViewOpen(!reportViewOpen)}
                        >
                              ...
                              {finalReport.history.map((r: any) => {
                                ...
                                return (
                                  <div key={r.id} className="flex items-center gap-3 p-2.5 rounded-lg bg-gray-50 text-sm">
                                    ...
                                    <Badge variant={st?.variant || 'neutral'} className="text-[10px]">
                                      {st?.label || r.status}
                                    </Badge>
              ...
              {reportViewOpen && (
                <SectionCard title="Báo cáo" icon="fa-solid fa-square-poll-vertical">
                  <div className="mt-4 border-t border-gray-100 pt-4">
                    ...
                    {pdfError && (
                      <p className="text-sm text-red-500 text-center py-4">{pdfError}</p>
                    )}
                    {reportPdfUrl && !loadingPdf && (
                      <PdfViewer fileUrl={reportPdfUrl} />
                    )}
                  </div>
                </SectionCard>
              )}
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
                      ...
                      <div className="p-3 rounded-xl bg-amber-50 border border-amber-100">
                        <div className="text-[11px] text-amber-600 font-medium uppercase tracking-wide">TB Hội đồng</div>
                        <div className="text-lg font-bold text-amber-800 mt-1">{currentGradeData.committee_avg ?? '—'}</div>
                      </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-[1fr_2fr_auto] gap-3 items-end">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          if (!isNaN(num) && selectedReg) handleSaveGrade(selectedReg.id, num, myComment);
                        }}
          ...
          <div className="space-y-6">
              ...
              {(regDetail.lecturer_info?.length ?? 0) > 0 ? (
                <div className="space-y-2">
                  {regDetail.lecturer_info.map((l: any) => {
                    const roleVariant = l.role === 'main' ? 'primary' : l.role === 'reviewer' ? 'warning' : 'neutral';
                    return (
                      <div key={l.id} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {l.academic_degree && (
                            <div className="text-xs text-gray-400 mt-0.5">{l.academic_degree}</div>
                          )}
                        </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 gap-1">
                ...
                {regDetail.wants_thesis_upgrade && (
                  <InfoTile icon="fa-solid fa-arrow-up" label="Nâng cấp khóa luận" value="Có đăng ký" />
                )}
